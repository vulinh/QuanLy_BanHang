...
<div class="row-fluid">
     <div class="span12">
        ...
        <td>
            <?php echo h($bill['Debited']['time']); ?>
            &nbsp;
        </td>
</tr>
<tr>        <td><strong><?php echo __('khác hàng'); ?></strong></td>
        <td>
            <?php echo $this->Html->link($bill['customer']['name'], array('controller' => 'users', 'action' => 'view', $bill['customer']['id']), array('class' => '')); ?>
            &nbsp;
        </td>
</tr>
        ...
        <td>
            <?php 
                if($bill['bills']['status'] == 0){
                    ...
                    echo '<br>'.$this->Html->link('Thanh toán', array('action' => 'xacnhanthanhtoan', $id), array('class'=>'')); 
                }else{
                    echo 'Đã thanh toán';
                }
            ?>
...
<?php foreach ($detailbills as $detailbill): ?>
    <tr>
        ...
        echo '<td>'.h($detailbill['Detailstock']['quantityExport']).'</td>';
        echo '<td>'.h($detailbill['Detailstock']['timeExport']).'</td>';
        ?>
    </tr>
<?php endforeach; ?>
                    </tbody>
