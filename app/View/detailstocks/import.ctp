...
<div class="row-fluid">
	...
	<table class="table table-striped table-bordered table-advance table-hover">
			...
			<?php echo $this->Form->select('Detailstock.1.idProduct',$dataProduct,array('class'=>"product_name")) ?>
				...
				<td style="width:50%; text-align:center;font-size:15px">
					<?php echo $this->Form->input('Detailstock.1.quatity', array('class'=>'product_quatity', 'label'=>false,'div'=>false, 'placeholder' => 'Số Lượng Nhập')); ?>
				</td>
...
<script type="text/javascript">
	...
	$('#btnAddRecord').click(function(event) {
		...
		$i = $i+1;
		$('#DetailstockImportForm table').append($('.control').last().clone());
	    
	    setElementName($('.product_name').last(),'data[Detailstock]['+$i+'][idProduct]');
	    setElementName($('.product_quatity').last(),'data[Detailstock]['+$i+'][quatity]');
	...
	$('#test').click(function(event) {
		if($('#DetailstockImportForm table tr').length >2){
		$i = $i-1;
		$('.control').last().remove();
		setElementName($('.product_name').last(),'data[Detailstock]['+$i+'][idProduct]');
		setElementName($('.product_quatity').last(),'data[Detailstock]['+$i+'][quatity]');
